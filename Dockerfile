...
RUN DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get install -y python-pip libsasl2-dev python-bzrlib python-pip bzr mercurial libxmlsec1-dev python-libxml2 python-openssl python-pydot python-cups python-dbus && \
    apt-get clean
...
RUN mkdir -p /.devstep/.ssh && chown -R developer:developer /.devstep/.ssh
